...
public class WorldSettingsMenu {
    ...
    public static void openMenu(Player player) {
        ...
        ItemStack wItem = new ItemStack(Plot.getWorldIcon(plot));
        ItemMeta meta = wItem.getItemMeta();
        meta.setDisplayName(Plot.getWorldName(plot));
        List<String> wLore = new ArrayList<>();
        wLore.add("§8ID: " + plot.world.getName());
        wLore.add("");
        wLore.add("§7" + Plot.getWorldDescription(plot));
        wLore.add("");
        wLore.add("§7 Онлайн: §f" + Plot.getPlayers(plot).size());
        wLore.add("§7 Создатель: §f" + Plot.getOwner(plot));
        wLore.add("§7");
