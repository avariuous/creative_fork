                ...
                if (player.isOnline()) {
                    ...
                    try {
                        ...
                        for (int configScoreboardIndex = 0; configScoreboardIndex < configScoreboardSize; configScoreboardIndex++) {
                            ...
                            Score score = objective.getScore(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getStringList("scoreboards.world.lines").get(configScoreboardIndex).replace("%player%",player.getName()).replace("%online%",String.valueOf(Bukkit.getOnlinePlayers().size())).replace("%worldOnline%",String.valueOf(Plot.getPlayers(plot).size())).replace("%owner%",plot.owner)));
